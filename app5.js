...
async function main() {
    ...
    try {
        ...
        await findListingByName(client, "harsh");
        // Update the Infinite Views listing to have 6 bedrooms and 8 beds 
        await updateListingByName(client, "harsh", { isActive: false, isVisible: true });
...
async function updateListingByName(client, nameOfListing, updatedListing) {
    ...
    const result = await client.db("database").collection("issues").updateOne({ name: nameOfListing }, { $set: updatedListing });
...
async function upsertListingByName(client, nameOfListing, updatedListing) {
    // See https://mongodb.github.io/node-mongodb-native/3.6/api/Collection.html#updateOne for the updateOne() docs
    const result = await client.db("database").collection("issues").updateOne({ name: nameOfListing }, { $set: updatedListing }, { upsert: true });
    console.log(`${result.matchedCount} document(s) matched the query criteria.`);
...
async function updateAllListingsToHavePropertyType(client) {
    ...
    const result = await client.db("database").collection("issues").updateMany({ property_type: { $exists: false } }, { $set: { property_type: "Unknown" } });
    console.log(`${result.matchedCount} document(s) matched the query criteria.`);
    console.log(`${result.modifiedCount} document(s) was/were updated.`);
}
